...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "മുകുന്ദപുരം സൊസൈറ്റി ലിമിറ്റഡ് — വായ്പകൾ, കുറികൾ & നിക്ഷേപങ്ങൾ" },
      { name: "description", content: "തൽക്ഷണ വായ്പകൾ, ആകർഷകമായ സ്ഥിര നിക്ഷേപങ്ങൾ, സ്വർണ്ണ വായ്പകൾ, വിശ്വസ്ത കുറികൾ." },
    ],
  }),
  component: HomePage,
});
...
function Hero() {
  ...
  return (
    <section className="relative bg-gradient-primary text-primary-foreground overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 py-16 md:py-24 grid lg:grid-cols-2 gap-10 items-center relative">
        <div>
          ...
          <p className="mt-5 text-xl opacity-90 max-w-lg">{t("home.hero.desc")}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link to="/apply" className="rounded-md bg-background text-primary px-6 py-3 font-semibold shadow-elegant hover:scale-[1.02] transition">
              {t("home.hero.apply")}
            </Link>
            <Link to="/about" className="rounded-md border border-primary-foreground/40 px-6 py-3 font-semibold hover:bg-primary-foreground/10 transition">
              {t("home.hero.learn")}
            </Link>
          </div>
        </div>
        <div className="relative">
          ...
          <img src={heroImg} alt="Financial growth" width={1280} height={896} className="relative rounded-2xl shadow-2xl w-full h-auto object-cover aspect-[4/3]" />
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="py-20 bg-navy text-navy-foreground">
      <div className="mx-auto max-w-7xl px-4 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold">{t("home.calc.title1")}<br /><span className="text-primary">{t("home.calc.title2")}</span></h2>
        ...
        <div className="rounded-2xl bg-white/5 backdrop-blur border border-white/10 p-7">
          ...
          <div className="mt-6 space-y-5">
            <Slider label={t("home.calc.amount")} value={fmt(amount)} min={50000} max={400000} step={10000} v={amount} setV={setAmount} />
            <Slider label={t("home.calc.tenure")} value={`${tenure} ${t("home.calc.months")}`} min={6} max={60} step={6} v={tenure} setV={setTenure} />
            <Slider label={t("home.calc.rate")} value={`${rate}${t("home.calc.rateUnit")}`} min={8} max={18} step={0.5} v={rate} setV={setRate} />
          </div>
  ...
  return (
    <section className="py-20 bg-gradient-gold text-gold-foreground">
      <div className="mx-auto max-w-7xl px-4 grid lg:grid-cols-2 gap-10 items-center">
        <img src={goldImg} alt="Gold loan" width={1024} height={768} loading="lazy" className="rounded-2xl shadow-2xl w-full h-auto object-cover aspect-[4/3]" />
        <div>
      ...
      <div className="mx-auto max-w-7xl px-4 grid md:grid-cols-2 gap-6">
        <div className="rounded-2xl bg-card border border-border shadow-card p-7">
          <div className="flex items-center gap-2 font-semibold text-lg">
            <span className="h-9 w-9 rounded-md bg-primary/15 text-primary grid place-items-center"><PiggyBank size={18} /></span>
            {t("home.fd.title")}
          </div>
          ...
          <div className="mt-5 rounded-lg bg-primary/10 border border-primary/20 p-4 space-y-1.5 text-base">
            <div className="flex justify-between"><span>{t("home.fd.rate")}</span><span className="font-semibold text-primary">{rate}% p.a.</span></div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="rounded-2xl bg-card border border-border shadow-card p-7">
          <div className="flex items-center gap-2 font-semibold text-lg">
            <span className="h-9 w-9 rounded-md bg-primary/15 text-primary grid place-items-center"><Phone size={18} /></span>
            {t("home.cb.title")}
          </div>
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {items.map((it) => (
            <div key={it.title} className="text-center group">
              <div className="mx-auto h-16 w-16 rounded-full bg-gradient-primary grid place-items-center text-primary-foreground shadow-elegant group-hover:scale-110 transition">
                <it.icon size={26} />
              </div>
...
function Testimonials() {
  const { t } = useT();
  const items = [
    { quote: t("home.test.q1"), name: "രാജേഷ് കുമാർ" },
    { quote: t("home.test.q2"), name: "പ്രിയ ശർമ്മ" },
    { quote: t("home.test.q3"), name: "അമിത് പട്ടേൽ" },
  ];
  return (
    <section className="py-20 bg-muted">
      <div className="mx-auto max-w-7xl px-4">
        <div className="text-center">
          <h2 className="text-3xl md:text-4xl font-bold">{t("home.test.title")}</h2>
          <p className="mt-2 text-muted-foreground">{t("home.test.desc")}</p>
        </div>
        <div className="mt-12 grid md:grid-cols-3 gap-6">
          {items.map((it) => (
            <div key={it.name} className="rounded-xl bg-card border border-border p-6 shadow-card">
              <div className="flex gap-0.5 text-gold">
                {Array.from({ length: 5 }).map((_, i) => <Star key={i} size={16} fill="currentColor" />)}
              </div>
              <p className="mt-3 text-base text-foreground/80">"{it.quote}"</p>
              <p className="mt-4 text-base font-semibold text-primary">— {it.name}</p>
            </div>
          ))}
        </div>
      </div>
